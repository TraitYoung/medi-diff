...
Generated images go to `outputs/generated/samples/<prefix>_<timestamp>_<seq>/`. The UI gallery scans batch dirs under `samples/`. The `--output-base` flag in `run_full_report.py` defaults to `outputs/generated/samples` so UI-visible batches stay consistent.
...
## Tuning bottleneck notes

When asked to fix image-quality tuning, follow `docs/private/TUNING_BOTTLENECK.md`: stabilize evaluation (strict vs calibrated) before changing generation parameters; fix `--source-seed` for A/B; one variable at a time.

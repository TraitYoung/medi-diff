...
| `app_gradio.py` | **日常主入口**：通过 `subprocess` 调用仓库内 `scripts/`，**不经由** HTTP 调用本机 API（与 API 并行、逻辑独立）。 |
...
**输出目录约定**：Gradio「生成」默认根目录为 `outputs/generated/samples/`。「一键流水线」经 `run_full_report.py` **显式传入同一 `--output-base`**，与画廊/评估下拉一致。若本地仍有旧目录 `outputs/generated/毕业论文_生成图像/`，请将批次目录迁移到 `samples/`。
...
- **单一流程页**：速度预设（快速 / 精细）、体位与 **密度（默认 `scattered`）** 等。界面固定走 **全图模式**，不展示 Patch 拼接、LoRA 路径、LoRA scale 或 Negative Prompt。
- **全图** 模式会传 `--mode full-image` 与 **`--fullimage-output-long-side 1024`**，便于 Gradio 预览与快速评估；留档大图时可在 CLI 中改回 2048。
- **质量调优默认**：快速预设 `strength=0.34, guidance=6.2`；精细预设 `strength=0.36, guidance=6.5`，优先保留 MLO 胸大肌、IMF 和乳腺条索结构。
- **LoRA** 由程序内部固定为 `outputs/lora/mammo_sd15_v6_allMLO/final_lora`；如需研究性切换权重或负向提示词，请使用 CLI/API。
